...
def preprocess_dataset_ljspeech(config, tts_engine: ChatterboxTTS):
    ...
    for idx, row in tqdm(data.iterrows(), total=len(data)):
        ...
        try:
            ...
            # Apply Romanian preprocessing if enabled (maps ș→sh, ț→ts, lowercase)
            # This replaces punc_norm for Romanian to avoid adding new tokens
            if config.romanian_preprocessing:
                clean_text = ro_preprocess(
                    raw_text,
                    lowercase=True,
                    mode=getattr(config, 'romanian_mode', 'phoneme'),
                )
            else:
