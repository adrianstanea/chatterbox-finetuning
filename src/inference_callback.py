...
class InferenceCallback(TrainerCallback):
    ...
    def on_save(self, args, state, control, **kwargs):
        ...
        # Only run inference on the main process (rank 0) in distributed training
    ...
    def _generate_sample(self, checkpoint_path: str, output_path: str):
        ...
        inference_text = self.config.inference_test_text
        if getattr(self.config, 'romanian_preprocessing', False):
            inference_text = ro_preprocess(
                inference_text,
                lowercase=True,
                mode=getattr(self.config, 'romanian_mode', 'phoneme'),
            )
            logger.info(f"Romanian preprocessed text: '{inference_text}'")
